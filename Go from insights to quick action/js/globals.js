// ----------------------------------------------------------------------------
// Custom Globals: Load embedUrl & reportId dynamically from reportList.json
// ----------------------------------------------------------------------------

// Report configuration (populated from JSON)
const reportConfig = {
  accessToken: null,
  embedUrl:    null,
  reportId:    null
};

// Session state (unchanged)
const reportShowcaseState = {
  report: null,
  data: null,
  allChecked: false,
  tooltipNextPressed: false
};

// DOM caches (unchanged)
const body               = $("#insight-to-action");
const embedContainer     = $("#report-container").get(0);
const overlay            = $("#overlay");
const distributionDialog = $("#distribution-dialog");
const dialogMask         = $("#dialog-mask");
const sendDialog         = $("#send-dialog");
const successDialog      = $("#success-dialog");
const closeBtn1          = $("#close1");
const closeBtn2          = $("#close2");
const sendCouponBtn      = $("#send-coupon");
const sendDiscountBtn    = $("#send-discount");
const sendMessageBtn     = $("#send-message");
const successCross       = $("#success-cross");

// CSS classes, keycodes, enums (unchanged)
const HIDE_OVERFLOW = "overflow-hidden";
let isDialogClosed = true;
const KEYCODE_TAB    = 9;
const KEYCODE_ESCAPE = 27;
const Keys = Object.freeze({ TAB: "Tab", ESCAPE: "Escape" });

// Visual GUID, icon, etc. (unchanged)
const TABLE_VISUAL_GUID = "1149606f2a101953b4ba";
let tableVisual;

// Icon for the custom extension
const base64Icon = "data:image/png;base64,"

// ----------------------------------------------------------------------------
// Step 1: Load reportList.json to populate reportConfig
// ----------------------------------------------------------------------------

// Determine desired report from URL (?report=Name)
const params     = new URLSearchParams(window.location.search);
const reportName = params.get("report") || null;

// Fetch the list (must be next to index.html)
fetch("../reportList.json")
  .then(res => {
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    return res.json();
  })
  .then(list => {
    // Pick by name or fallback to first
    let item = reportName && list.find(r => r.name === reportName);
    if (!item) item = list[0];

    if (!item || !item.embedUrl) {
      throw new Error(`Report not found: ${reportName}`);
    }

    // Populate config for index.js embedReport()
    reportConfig.embedUrl = item.embedUrl;
    // Extract reportId query param from the embedUrl
    const url = new URL(item.embedUrl);
    reportConfig.reportId = url.searchParams.get("reportId");

    // Now the embedReport() in index.js can run using this config
    // It’s intentionally called from index.js on document.ready.
  })
  .catch(err => {
    console.error("❌ reportList.json error:", err);
    overlay.innerHTML = `<div style="color:red; padding:20px;">
      Failed to load report list:<br>${err.message}
    </div>`;
  });
// ----------------------------------------------------------------------------
// Step 1: Load reportList.json to populate reportConfig (end)
// ----------------------------------------------------------------------------
const distributionDialogElements = {
    firstElement: closeBtn1,
    lastElement: sendDiscountBtn
}

const sendDialogElements = {
    firstElement: closeBtn2,
    lastElement: sendMessageBtn
}

const successDialogElements = {
    firstElement: successCross,
    lastElement: successCross
}
